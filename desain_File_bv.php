...
$desain_File_blobview = NULL; // Initialize page object first

class cdesain_File_blobview extends cdesain {
	...
	// Page object name
	var $PageObjName = 'desain_File_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'desain');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('desain', 'x_File');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('File');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('PCBName')));
				if (trim(strval($this->Recordset->fields('PCBName'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('PCBName') . "\"");
				}
...
// Create page object
if (!isset($desain_File_blobview)) $desain_File_blobview = new cdesain_File_blobview();

// Page init
$desain_File_blobview->Page_Init();

// Page main
$desain_File_blobview->Page_Main();
?>
<?php
$desain_File_blobview->Page_Terminate();
?>
