...
$desain_edit = NULL; // Initialize page object first

class cdesain_edit extends cdesain {
	...
	function Page_Main() {
		...
		if ($this->OrderID->CurrentValue == "")
			$this->Page_Terminate("desainlist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		$this->StatusOrderID->ViewCustomAttributes = "";
			...
			$sWhereWrk = "";
			ew_AddFilter($sWhereWrk, $sFilterWrk);
			if (!$GLOBALS["desain"]->UserIDAllow("edit")) $sWhereWrk = $GLOBALS["user"]->AddUserIDFilter($sWhereWrk);
			$this->Lookup_Selecting($this->_UserID, $sWhereWrk); // Call Lookup selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			$sSqlWrk .= " ORDER BY `UserID` ASC";
			$rswrk = Conn()->Execute($sSqlWrk);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->MakeFrom->SetDbValueDef($rsnew, $this->MakeFrom->CurrentValue, "", $this->MakeFrom->ReadOnly);

			// AmountOfComponents
			$this->AmountOfComponents->SetDbValueDef($rsnew, $this->AmountOfComponents->CurrentValue, 0, $this->AmountOfComponents->ReadOnly);

			// Program
			$this->Program->SetDbValueDef($rsnew, $this->Program->CurrentValue, "", $this->Program->ReadOnly);

			// TotalI/O
			$this->TotalI2FO->SetDbValueDef($rsnew, $this->TotalI2FO->CurrentValue, 0, $this->TotalI2FO->ReadOnly);

			// TotalFunction
			$this->TotalFunction->SetDbValueDef($rsnew, $this->TotalFunction->CurrentValue, 0, $this->TotalFunction->ReadOnly);
			...
			// DaysOfWorks
			$this->DaysOfWorks->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->DaysOfWorks->CurrentValue, 7), ew_CurrentDate(), $this->DaysOfWorks->ReadOnly);

			// TotalDesignCost
			$this->TotalDesignCost->SetDbValueDef($rsnew, $this->TotalDesignCost->CurrentValue, 0, $this->TotalDesignCost->ReadOnly);

			// StatusOrderID
			$this->StatusOrderID->SetDbValueDef($rsnew, $this->StatusOrderID->CurrentValue, 0, $this->StatusOrderID->ReadOnly);
...
// Create page object
if (!isset($desain_edit)) $desain_edit = new cdesain_edit();

// Page init
$desain_edit->Page_Init();

// Page main
$desain_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$desain_edit->Page_Render();
?>
...
var CurrentForm = fdesainedit = new ew_Form("fdesainedit", "edit");

// Validate form
fdesainedit.Validate = function() {
...
fdesainedit.Form_CustomValidate = 
...
fdesainedit.ValidateRequired = true;
<?php } else { ?>
fdesainedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdesainedit.Lists["x__UserID"] = {"LinkField":"x__UserID","Ajax":true,"AutoFill":false,"DisplayFields":["x_NamaLengkap","x_NomorHP","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdesainedit.Lists["x_MakeFrom"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdesainedit.Lists["x_MakeFrom"].Options = <?php echo json_encode($desain->MakeFrom->Options()) ?>;
fdesainedit.Lists["x_Program"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fdesainedit.Lists["x_Program"].Options = <?php echo json_encode($desain->Program->Options()) ?>;
fdesainedit.Lists["x_StatusOrderID"] = {"LinkField":"x_StatusOrderID","Ajax":true,"AutoFill":false,"DisplayFields":["x_StatusOrder","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $desain_edit->ShowPageHeader(); ?>
<?php
$desain_edit->ShowMessage();
?>
<form name="fdesainedit" id="fdesainedit" class="<?php echo $desain_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($desain_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $desain_edit->Token ?>">
<?php } ?>
...
<?php if ($desain->OrderID->Visible) { // OrderID ?>
	<div id="r_OrderID" class="form-group">
		<label id="elh_desain_OrderID" class="col-sm-2 control-label ewLabel"><?php echo $desain->OrderID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $desain->OrderID->CellAttributes() ?>>
<span id="el_desain_OrderID">
<span<?php echo $desain->OrderID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $desain->OrderID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="desain" data-field="x_OrderID" name="x_OrderID" id="x_OrderID" value="<?php echo ew_HtmlEncode($desain->OrderID->CurrentValue) ?>">
<?php echo $desain->OrderID->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if (!$Security->IsAdmin() && $Security->IsLoggedIn() && !$desain->UserIDAllow("edit")) { // Non system admin ?>
...
<script type="text/javascript">
ew_CreateCalendar("fdesainedit", "x_DaysOfWorks", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $desain_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
