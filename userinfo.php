...
class cuser extends cTable {
	...
	function __construct() {
		...
		// NamaLengkap
		$this->NamaLengkap = new cField('user', 'user', 'x_NamaLengkap', 'NamaLengkap', '`NamaLengkap`', '`NamaLengkap`', 200, -1, FALSE, '`NamaLengkap`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['NamaLengkap'] = &$this->NamaLengkap;

		// NomorHP
		$this->NomorHP = new cField('user', 'user', 'x_NomorHP', 'NomorHP', '`NomorHP`', '`NomorHP`', 200, -1, FALSE, '`NomorHP`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['NomorHP'] = &$this->NomorHP;

		// Alamat
		$this->Alamat = new cField('user', 'user', 'x_Alamat', 'Alamat', '`Alamat`', '`Alamat`', 201, -1, FALSE, '`Alamat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->NamaLengkap->EditValue = $this->NamaLengkap->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->NamaLengkap->Exportable) $Doc->ExportCaption($this->NamaLengkap);
					if ($this->NomorHP->Exportable) $Doc->ExportCaption($this->NomorHP);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->NamaLengkap->Exportable) $Doc->ExportField($this->NamaLengkap);
						if ($this->NomorHP->Exportable) $Doc->ExportField($this->NomorHP);
	...
	function UserIDFilter($userid) {
		...
		$sParentUserIDFilter = '`UserID` IN (SELECT `UserID` FROM ' . "`user`" . ' WHERE `UserID` = ' . ew_QuotedValue($userid, EW_DATATYPE_NUMBER, EW_USER_TABLE_DBID) . ')';
